                      ...
                      Quadrotor::InternalState& dxdt, const double /* t */)
{
  ...
  x_dot = cur_state.v;
  v_dot = Eigen::Vector3d(0.0, 0.0, -g_)                           // 重力（惯性坐标系 z 轴向上）
          + (thrust / mass_) * (R * Eigen::Vector3d(0.0, 0.0, 1.0)) // 推力（机体系 z 轴沿 R 变换到惯性系）
          + external_force_ / mass_                                 // 外力
          - (resistance / mass_) * vnorm;                           // 阻力（与速度反向）
